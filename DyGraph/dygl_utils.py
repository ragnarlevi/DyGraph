...
def inner_em(X,A, theta, nu, eta, gamma, A_gamma = None, rho_gamma = 0, groups = None, lik_type = "t", nr_itr = 5, tol = 1e-5, nr_quad = 5, pool = None):
    ...
    if lik_type == "t":
        ...
        while iteration < nr_itr:
            ...
            for i in range(x.shape[0]):
                D_1 = D1_EM(theta_pre,x[i],nu,groups, gamma_pre, nr_quad)
                D_2 = D2_EM(theta_pre,x[i],nu,groups, gamma_pre, nr_quad)
                D_3 = D3_EM(theta_pre,x[i],nu,groups, gamma_pre, nr_quad)


                S +=  (np.multiply(np.outer(x[i],x[i]),D_1 )-
                       np.multiply(np.outer(gamma_pre,x[i]),D_2 )-
                       np.multiply(np.outer(x[i],gamma_pre),D_2.T )+
                       np.multiply(np.outer(gamma_pre,gamma_pre),D_3 ) )/float(x.shape[0])
                
                G1 += np.multiply(theta_pre, D_2).dot(x[i])
                G2 += D_3
            # M-step
            theta_new = Gaussian_update(S, A, eta)
...
def skew_group_t_EM_iteration(x_i,theta_pre, nu, groups, gamma_pre,nr_quad):
...
def block_wise_reconstruction(A,  eta ):
    """
    ...
    if eta > 0.0:
        ...
        for i in range(A.shape[1]):
            ...
            l_opt = optimize.bisect(f = f, a = 0, b = np.sum(np.abs(A[:,i]/eta)), args = (A[:,i]/eta,), xtol=1e-5)
