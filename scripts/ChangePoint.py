        ...
        for nr_obs_per_graph in [100]:
            ...
            for i in range(len(alpha)):
                for j in range(len(kappa)):
                    ...
                    dg_opt1 = dg.dygl_parallel(obs_per_graph = obs_per_graph_model, max_iter = 2000, lamda = obs_per_graph_model*alpha[i], kappa = obs_per_graph_model*kappa[j], tol = 1e-10)
                    dg_opt1.fit(Xs, nr_workers=12, temporal_penalty=temporal_penalty, lik_type="gaussian", verbose=False)
            ...
            with open(f'data/single/single_change_gaussian_{temporal_penalty}_{n}_{obs_per_graph_model}_1e-10.pkl', 'wb') as handle:
