...
from DyGraph import sgl_inner_em
...
def test_sgl_inner_em():
    ...
    dg_opt = sgl_inner_em(X, max_iter = 2000, lamda = alpha, tol = tol)
    ...
    dg_opt = sgl_inner_em(X, max_iter = 2000, lamda = alpha, tol = tol, lik_type='t')
    dg_opt.fit(nu = None,verbose=True,  nr_em_itr = 1)
    assert_positive_eig(dg_opt.theta[-1])
    ...
    dg_opt = sgl_inner_em(X, lik_type='group-t', max_iter = 200, lamda = alpha, tol = tol_integrate, groups = [0]*d)
    dg_opt.fit(nu = None, verbose=True,  nr_em_itr = 1)
    assert_positive_eig(dg_opt.theta[-1])
    ...
    dg_opt = sgl_inner_em(X, lik_type='skew-group-t', max_iter = 200, lamda = alpha, tol = tol_integrate, groups = [0]*d)
